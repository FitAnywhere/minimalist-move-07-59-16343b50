        ...
        <div className="max-w-4xl mx-auto">
          <div className={cn("text-center transition-all duration-700", isInView ? "opacity-100" : "opacity-0 translate-y-8")}>
            <h2 className="text-black">
              Introducing PowerTower: 
              <span className="block mt-2">TURN ANY SPACE INTO YOUR GYM</span>
            </h2>
            
            <p className="mt-6 text-xl">Join the personal fitness revolution and gain the freedom to train on your terms.</p>
          </div>
          
          <div className="mt-12">
            <div className={cn("relative overflow-hidden transition-all duration-500", expanded ? "max-h-[1000px]" : "max-h-0")}>
              <div className="grid md:grid-cols-2 gap-8 mb-8">
                {features.map((feature, index) => <div key={index} className={cn("bg-gray-50 p-6 rounded-2xl transition-all", "hover:shadow-md hover:-translate-y-1 duration-300")} style={{
              ...
              <div className="aspect-video max-w-3xl mx-auto rounded-2xl overflow-hidden">
                <img src="https://images.unsplash.com/photo-1486718448742-163732cd1544" alt="PowerTower Minimalist Design" className="w-full h-full object-cover" />
              </div>
            </div>
            
            <button onClick={() => setExpanded(!expanded)} className="mt-8 flex items-center justify-center mx-auto bg-white border border-gray-200 px-6 py-3 rounded-full text-sm font-medium hover:bg-gray-50 transition-all hover-lift">
              {expanded ? <>Show Less <ChevronUp className="ml-2 w-4 h-4" /></> : <>Read More <ChevronDown className="ml-2 w-4 h-4" /></>}
            </button>
          </div>
        </div>
      </div>
