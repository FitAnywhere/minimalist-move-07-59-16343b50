...
const TargetAndFAQ = () => {
  ...
  return <section id="faq" ref={sectionRef} className="py-24 bg-white">
          ...
          <div className={cn("text-center mb-20 transition-all duration-700", isInView ? "opacity-100" : "opacity-0 translate-y-8")}>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {targetAudiences.map((audience, index) => <div key={index} className={cn("bg-gray-50 rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 hover:bg-white hover:shadow-lg", activeAudience === index ? "bg-white shadow-lg" : "")} onClick={() => toggleAudience(index)}>
                  ...
                  <div className={cn("overflow-hidden transition-all duration-300", activeAudience === index ? "max-h-40 opacity-100" : "max-h-0 opacity-0")}>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              transitionDelay: `${index * 100}ms`
                  ...
                  <button onClick={() => toggleFAQ(index)} className="w-full flex justify-between items-center text-left py-4 focus:outline-none">
                    ...
                    <div className={cn("flex-shrink-0 w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center transition-transform duration-300", openFAQ === index ? "bg-yellow transform rotate-180" : "")}>
