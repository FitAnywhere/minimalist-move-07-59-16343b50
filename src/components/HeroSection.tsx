        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div className="text-center md:text-left order-2 md:order-1">
            <h1 
              ref={headlineRef}
              className={cn(
                "transition-all duration-1000 font-bold text-black",
            ...
            <p 
              ref={subheadlineRef}
              className={cn(
                "mt-6 text-xl md:text-2xl transition-all duration-1000 delay-200 text-gray-800",
                isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              )}
            >
          ...
          {/* Right Column - Videos */}
          <div className={cn(
            "order-1 md:order-2 transition-all duration-1000 delay-300",
            isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          )}>
            ...
            <div className="hidden md:grid grid-cols-2 gap-4">
              {videos.map((video) => (
                <div 
                  key={video.id}
                  className="relative group cursor-pointer rounded-xl overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
        ...
        <DialogContent className="max-w-4xl w-[90vw] h-[80vh] p-0 bg-black border-none">
          ...
          {videoPlaying && (
            <div className="w-full h-full">
              <iframe 
                src={videos.find(v => v.id === videoPlaying)?.url} 
                width="100%" 
                height="100%" 
                allow="autoplay; encrypted-media" 
                allowFullScreen
                title={videos.find(v => v.id === videoPlaying)?.title}
