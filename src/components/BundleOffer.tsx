  ...
  return <section id="bundle" ref={sectionRef} className="relative overflow-hidden py-16 bg-white">
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className={cn("mb-10 transition-all duration-1000", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10")}>
            <div className="relative">
              <div className="overflow-hidden" ref={emblaRef}>
                <div className="flex">
                  {products.map((product, index) => <div key={index} className="flex-[0_0_100%] min-w-0 sm:flex-[0_0_50%] md:flex-[0_0_66.66%] lg:flex-[0_0_50%] px-2 sm:px-4">
                      <div className={cn("rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg", "h-full bg-gradient-to-br relative", "bg-gradient-to-br from-yellow-50/10 to-white/5", "before:absolute before:inset-0 before:bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI2MCIgaGVpZ2h0PSI2MCIgdmlld0JveD0iMCAwIDYwIDYwIiB6PjxwYXRoIGQ9Ik0wIDBhNyA3IDAgMDEwIDE0QTcgNyAwIDAxMCAwWiIgZmlsbD0iI2YxZjFmMSIgZmlsbC1vcGFjaXR5PSIwLjIiPjwvcGF0aD48L3N2Zz4=')] before:opacity-10", "after:absolute after:inset-0 after:bg-yellow-50/10 after:backdrop-blur-[1px]", "hover:translate-y-[-5px] hover:shadow-xl", "shadow-[0_15px_50px_rgba(255,215,0,0.4)]")}>
                        <div className="relative z-10">
                          <div className="h-72 md:h-80 flex items-center justify-center p-4 group">
                            ...
                            <div className="relative w-full h-full flex items-center justify-center">
                              ...
                              <img src={product.image} alt={product.name} className="w-full h-full object-contain transition-transform duration-500 group-hover:scale-105 relative z-10 drop-shadow-md" />
                            </div>
                          </div>
                          <div className="p-6 pt-0 text-center relative z-10">
                            <div className="flex items-center justify-center gap-2 mb-2">
                              ...
                              {product.badge && <Badge className="bg-yellow text-black text-xs font-bold py-1">
                                  {product.badge}
                                </Badge>}
                            </div>
              ...
              <button onClick={() => emblaApi?.scrollPrev()} className="absolute -left-5 md:-left-10 top-1/2 transform -translate-y-1/2 bg-white text-black border border-gray-200 rounded-full p-3 shadow-md hidden md:flex items-center justify-center z-10 hover:bg-yellow-400 hover:text-black transition-all duration-300" aria-label="Previous slide">
                <ArrowLeft className="h-6 w-6" />
              </button>
              
              <button onClick={() => emblaApi?.scrollNext()} className="absolute -right-5 md:-right-10 top-1/2 transform -translate-y-1/2 bg-white text-black border border-gray-200 rounded-full p-3 shadow-md hidden md:flex items-center justify-center z-10 hover:bg-yellow-400 hover:text-black transition-all duration-300" aria-label="Next slide">
                <ArrowRight className="h-6 w-6" />
              </button>
            </div>
            
            <div className="flex justify-center gap-4 mt-4 md:hidden">
              <button onClick={() => emblaApi?.scrollPrev()} className="bg-white border border-gray-200 hover:bg-yellow-400 hover:text-black rounded-full p-3 shadow-sm transition-all duration-300" aria-label="Previous slide">
                <ArrowLeft className="h-5 w-5" />
              </button>
              <button onClick={() => emblaApi?.scrollNext()} className="bg-white border border-gray-200 hover:bg-yellow-400 hover:text-black rounded-full p-3 shadow-sm transition-all duration-300" aria-label="Next slide">
                <ArrowRight className="h-5 w-5" />
              </button>
            </div>
            
            <div className="mt-4 max-w-xl mx-auto px-4">
              <Progress value={scrollProgress} className="h-1.5 bg-gray-200 rounded-full" />
            </div>
          </div>
          
          <div className={cn("mb-10 transition-all duration-1000 delay-300", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10")}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
              {giftItems.map((item, index) => <div key={index} className="rounded-lg bg-white p-4 border border-[#13613A]">
                  <div className="flex items-center">
                    <div className="w-20 h-20 flex-shrink-0 mr-4 overflow-hidden">
                      <img src={item.image} alt={item.name} className="w-full h-full object-contain transition-all duration-300 hover:scale-110" />
                    </div>
                    <div>
          ...
          <div className={cn("mb-8 transition-all duration-1000 delay-500", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")}>
            {isMobile ? <div className="flex flex-col items-center justify-center">
                <span className="text-xl text-gray-700 line-through mb-1">
                  {isVisible ? <>
                      €<CountUp start={0} end={originalPrice} duration={2} separator="," decimals={2} decimal="," />
                    </> : `€${originalPrice}`}
                </span>
