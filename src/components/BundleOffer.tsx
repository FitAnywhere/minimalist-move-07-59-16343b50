...
const BundleOffer = () => {
  ...
  const isBonusInView = useInView(bonusSectionRef, {
    threshold: 0.5
  });
  ...
  return <section id="bundle" ref={sectionRef} className="py-24 relative overflow-hidden">
      {/* Parallax Background */}
      <div ref={parallaxRef} className="absolute inset-0 w-full h-full z-0 overflow-hidden" style={{
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className={cn("text-center transition-all duration-1000 transform", isInView ? "opacity-100 translate-y-0 animate-[bounce_0.5s_ease-out]" : "opacity-0 -translate-y-8")}>
          ...
          {/* Bundle Offer Card */}
          <div className={cn("mt-10 bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100 transition-all duration-1000", isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10")}>
            <div className="p-8">
              ...
              <div ref={bonusSectionRef} className={cn("mt-10 p-6 rounded-xl border border-yellow-100 transition-all duration-500", isBonusInView ? "bg-yellow/5" : "bg-white")}>
                <h3 className={cn("text-xl font-bold mb-4 flex items-center", isBonusInView ? "animate-[pulse_2s_infinite]" : "")}>
                  🎁 EXCLUSIVE BONUSES:
                </h3>
                
                <div className="space-y-4">
                  {bundleItems.filter(item => item.isBonus).map((item, index) => <div key={index} className={cn("flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-white transition-all duration-500 transform", "hover:shadow-md border border-yellow-50", isBonusInView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-12")} style={{
                  transitionDelay: `${index * 200}ms`
                      ...
                      <div className="mt-3 sm:mt-0 text-right">
                        <span className="font-semibold text-xl text-green-500 transition-all duration-300 hover:text-shadow-yellow">
                          {item.price}
                        </span>
                      </div>
                    </div>)}
            ...
            <div className="bg-gray-900 p-8 text-center">
              <Button size="lg" className={cn("bg-yellow hover:bg-yellow-dark text-black px-8 py-6 rounded-full text-lg font-semibold tracking-wide", "transition-all duration-300 hover:shadow-lg hover:-translate-y-1", "animate-[pulse_3s_infinite] hover:animate-none", "w-full sm:w-auto")}>
